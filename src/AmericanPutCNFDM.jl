function AmericanPutCNFDM(K, T, S, sig, r, div, N, Nj, dx)
...
function solve_Crank_Nicolson_tridiagonal_system(C, pu, pm, pd, lambda_L, lambda_U)
...
pmp[J[-Nj]+1] = pm + pd
pp[J[-Nj]+1] = -pu * C[J[-Nj]+2, 1] - (pm -2) * C[J[-Nj]+1, 1] - pd*C[J[-Nj]] + pd * lambda_L

## eliminate upper diagonal
for j = -Nj+2:Nj-1
  pmp[J[j]] = pm - pu * pd / pmp[J[j]-1]
  pp[J[j]] = -pu * C[J[j]+1, 1] -(pm-2) * C[J[j], 1] - pd * C[J[j]-1, 1] - pp[J[j]-1] * pd / pmp[J[j]-1]
end
...
## as it only overwrites the first two cols
  ...
  for i = N-1:-1:0
    ...
    solve_Crank_Nicolson_tridiagonal_system(C, pu, pm, pd, lambda_L, lambda_U)
